...
{
    public class modify_mirror_control
    {
        ...
        public void modify_mirror(workarea_control wkc)
        {
            ...
            {
                set_mirror_of_lines(rotation_pt, rot_angle_rad);
            }

            // Set mirror arcs
            if (this.wkc_obj.interim_obj.selected_arcs.Count > 0)
            {
                set_mirror_of_arcs(rotation_pt, rot_angle_rad);
            }

            // Set mirror bezier
            if (this.wkc_obj.interim_obj.selected_beziers.Count > 0)
            {
                set_mirror_of_beziers(rotation_pt, rot_angle_rad);
            }

            // Delete member (if duplicate is not true)
            if (duplicate_transformation == false && (this.wkc_obj.interim_obj.selected_lines.Count > 0 || this.wkc_obj.interim_obj.selected_arcs.Count > 0))
            {
            ...
            {
                apply_mirror_of_lines();
            }

            // Apply mirror arcs
            if (this.selected_arc_count > 0)
            {
                apply_mirror_of_arcs();
            }

            // Apply Rotate beziers
            if (this.selected_bezier_count > 0)
            {
                apply_mirror_of_beziers();
            }
        }

        private void set_mirror_of_lines(PointF rotation_pt, double rot_angle_rad)
        {
        ...
        private void apply_mirror_of_lines()
        {
        ...
        private void set_mirror_of_arcs(PointF rotation_pt, double rot_angle_rad)
        {
            // Set the translation to click points
            ...
            foreach (arcs_store selected_arc in this.wkc_obj.interim_obj.selected_arcs)
            {
                ...
                // Pt1
                double rot_x1 = ((chord_s_pt.x - rotation_pt.X) * Math.Cos(2*rot_angle_rad) + (chord_s_pt.y - rotation_pt.Y) * Math.Sin(2*rot_angle_rad)) + rotation_pt.X;
                double rot_y1 = ((chord_s_pt.x - rotation_pt.X) * Math.Sin(2*rot_angle_rad) - (chord_s_pt.y - rotation_pt.Y) * Math.Cos(2*rot_angle_rad)) + rotation_pt.Y;

                // Pt2
                double rot_x2 = ((chord_e_pt.x - rotation_pt.X) * Math.Cos(2*rot_angle_rad) + (chord_e_pt.y - rotation_pt.Y) * Math.Sin(2*rot_angle_rad)) + rotation_pt.X;
                double rot_y2 = ((chord_e_pt.x - rotation_pt.X) * Math.Sin(2*rot_angle_rad) - (chord_e_pt.y - rotation_pt.Y) * Math.Cos(2*rot_angle_rad)) + rotation_pt.Y;
                ...
                // Pt3
                double rot_x3 = ((arc_cntrl_pt.X - rotation_pt.X) * Math.Cos(2*rot_angle_rad) + (arc_cntrl_pt.Y - rotation_pt.Y) * Math.Sin(2*rot_angle_rad)) + rotation_pt.X;
                double rot_y3 = ((arc_cntrl_pt.X - rotation_pt.X) * Math.Sin(2*rot_angle_rad) - (arc_cntrl_pt.Y - rotation_pt.Y) * Math.Cos(2*rot_angle_rad)) + rotation_pt.Y;

                // Pt4
                double rot_x4 = ((arc_center_pt.X - rotation_pt.X) * Math.Cos(2*rot_angle_rad) + (arc_center_pt.Y - rotation_pt.Y) * Math.Sin(2*rot_angle_rad)) + rotation_pt.X;
                double rot_y4 = ((arc_center_pt.X - rotation_pt.X) * Math.Sin(2*rot_angle_rad) - (arc_center_pt.Y - rotation_pt.Y) * Math.Cos(2*rot_angle_rad)) + rotation_pt.Y;
        ...
        private void apply_mirror_of_arcs()
        {
        ...
        private void set_mirror_of_beziers(PointF rotation_pt, double rot_angle_rad)
        {
            // Set the mirror to click points
            // Points list order [0,1], [2,3], [4,5] ...
            this.selected_bezier_count = 0;
            ...
            foreach (beziers_store selected_bezier in this.wkc_obj.interim_obj.selected_beziers)
            {
                ...
                for (int i = 0; i < selected_bezier.bezier_cntrl_pts.Count; i++)
                {
                    ...
                    rot_x1 = ((e_pt.X - rotation_pt.X) * Math.Cos(2*rot_angle_rad) + (e_pt.Y - rotation_pt.Y) * Math.Sin(2*rot_angle_rad)) + rotation_pt.X;
                    rot_y1 = ((e_pt.X - rotation_pt.X) * Math.Sin(2*rot_angle_rad) - (e_pt.Y - rotation_pt.Y) * Math.Cos(2*rot_angle_rad)) + rotation_pt.Y;
        ...
        private void apply_mirror_of_beziers()
        {
